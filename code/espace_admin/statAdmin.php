...
// Stat par garage
$sqlGarage = "SELECT g.nom,COUNT(i.id_intervention) as nb_interventions,COALESCE(AVG(i.cout), 0) as cout_moyen,COALESCE(SUM(i.cout), 0) as chiffre_affaires
    FROM garage g
...
<div class="container">
    ...
    <div class="dashboard-grid">
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="width: 30%;">Part du CA global</th>
                    </tr>
                </thead>
                <tbody>
                    <?php 
                    $maxCA = 0;
                    foreach($statsGarage as $g) $maxCA += $g['chiffre_affaires'];
                    
                    foreach ($statsGarage as $garage): 
                        $percent = ($maxCA > 0) ? ($garage['chiffre_affaires'] / $maxCA) * 100 : 0;
                    ?>
                    <tr>
                        <td><strong><?= htmlspecialchars($garage['nom']) ?></strong></td>
                        <td style="text-align:center;"><span class="tag tag-blue"><?= $garage['nb_interventions'] ?></span></td>
            ...
            <table>
                ...
                <tbody>
                    <?php foreach ($statsModele as $mod): 
                        // Calcul d'un "Ratio de dommage"
                        $ratio = ($mod['nb_visites'] > 0) ? $mod['total_pieces_remplacees'] / $mod['nb_visites'] : 0;
                        // Calcul "Indice santé" (Arbitraire pour l'exemple : 100% - (10% par pièce changée en moyenne))
        ...
        <div class="card">
            <h2>Top Composants Remplacés</h2>
